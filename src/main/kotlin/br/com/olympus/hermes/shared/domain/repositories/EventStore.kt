...
interface EventStore {
    /**
     * Appends new event envelopes for an aggregate. Uses optimistic concurrency control — the
     * operation fails if events already exist at or beyond [expectedVersion].
     *
     * @param aggregateId The aggregate's unique identifier (partition key).
     * @param events The uncommitted event envelopes to persist, ordered by version.
     ...
     * @return Either a [BaseError] on failure, or the ordered list of envelopes (empty if aggregate
     * does not exist).
     */
    fun getEvents(aggregateId: EntityId): Either<BaseError, List<EventWrapper>>
}
