 ...
 * aggregate state (Event Sourcing). Implementations must guarantee ordering by aggregate version and
 * support optimistic concurrency via [expectedVersion].
 */
interface EventStore {
    /**
     * Appends new events for an aggregate. Uses optimistic concurrency control — the operation fails
     * if events already exist at or beyond [expectedVersion].
     *
    ...
    /**
     * Retrieves events for an aggregate starting after a given version, ordered by version
     * ascending. Useful for partial replay when a snapshot is available.
     *
     * @param aggregateId The aggregate's unique identifier.
