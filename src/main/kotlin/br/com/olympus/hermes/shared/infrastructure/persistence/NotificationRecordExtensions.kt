...
internal fun Raise<BaseError>.readEntityId(record: NotificationRecord): EntityId = EntityId.from(record.id).bind()

/** Reads a required nullable field, raising a [PersistenceError] if it is null. */
internal fun <T> Raise<BaseError>.requireField(
    value: T?,
    fieldName: String,
    type: String,
): T = value ?: raise(PersistenceError("Missing $fieldName for $type record"))
